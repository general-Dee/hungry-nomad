...
## USER
- Lint config: `.eslintrc.json` — extends `next/core-web-vitals` + `@typescript-eslint/recommended`. `no-explicit-any` and unescaped-entities checks are relaxed; unused vars are warn-only.
- Stack: Next.js 14 App Router, TypeScript, Supabase, Paystack (payments), Resend (email), Upstash Redis (rate limiting), Sentry.
- Be paranoid about: price/amount tampering (order totals must be derived server-side, never trust client-submitted amounts — this repo was already burned by this once), auth/authorization on API routes under `src/app/api`, and secrets/env vars leaking into client code or logs.
